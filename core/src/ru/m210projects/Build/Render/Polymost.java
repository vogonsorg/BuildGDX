...
//	private int r_downsizevar = -1;
	...
	private final TSurface[] domost = new TSurface[4];
			...
			for (z = 0; z <= scnt; z++, i = vcnt) {
				...
				if (!dir) {
					...
					case 2: 
						...
						vsp[i].cy[0] = ny0;
						vsp[i].ctag = gtag;
						drawpoly(domost, 3, domostpolymethod);
						break;
					case 3:
					case 6:
						domost[0].px = dx0;
					...
					case 7:
						...
						vsp[i].cy[1] = ny1;
						vsp[i].ctag = gtag;
						drawpoly(domost, 4, domostpolymethod);
						break;
					case 8:
						domost[0].px = dx0;
						domost[0].py = vsp[i].cy[0];
...
//		gl.glDepthFunc(GL10.GL_ALWAYS); // NEVER,LESS,(,L)EQUAL,GREATER,(NOT,G)EQUAL,ALWAYS
//		gl.glPolygonOffset(0, 0);
		

//		gl.glDepthRange(0.0001, 1.0); // <- this is more widely supported than glPolygonOffset
...
//	private final int ROTATESPRITE_MAX = 2048;
	...
	public static boolean usenewcode;
	...
	public void dorotatesprite(int sx, int sy, int z, int a, int picnum,
		...
		if(!usenewcode) {
			double d = z / (65536.0f * 16384.0f);
			double cosang, sinang;
			double cosang2 = cosang = sintable[(a + 512) & 2047] * d;
			double sinang2 = sinang = sintable[a & 2047] * d;
			if (((dastat & 2) != 0) || ((dastat & 8) == 0)) // Don't aspect unscaled perms
			{
		...
		if(!usenewcode) {
			...
			do {
				...
				if ((d < x2) != (d < 0)) {
					drot[nn].px2 = fx;
					drot[nn].py2 = (drot[zz].py - drot[z].py) * d / x2 + drot[z].py;
					nn++;
				}
			...
			if (nn >= 3) {
				...
				do {
					...
					if ((d < y2) != (d < 0)) {
						drot[n].py = fy;
						drot[n].px = (drot[zz].px2 - drot[z].px2) * d / y2 + drot[z].px2;
						n++;
					}
