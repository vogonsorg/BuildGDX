...
public class JoglInput implements BuildInput, MouseListener, KeyListener {
	...
	public void init(BuildFrame frame) {
		GLWindow window = ((JoglGraphics) frame.getGraphics()).canvas;
		setListeners(window);
	}
...
//		canvas.setPointerVisible(visible); XXX
	}
...
//		showCursor(canvas.isActive && canvas.isFocused);
		return false;
	}
...
//			event.scrollAmount = -1.0f * e.getRotation()[1]; XXX
			event.timeStamp = System.nanoTime();
			touchEvents.add(event);
		}
	}
...
//			}
	...
	public void keyReleased (com.jogamp.newt.event.KeyEvent e) {
		synchronized (this) {
//			if(e.isAutoRepeat()&&e.isPrintableKey()) { XXX
//				KeyEvent event = usedKeyEvents.obtain();
//				event.keyChar = e.getKeyChar();;
//				event.keyCode = translateKeyCode(e.getKeyCode());
//				event.type = KeyEvent.KEY_TYPED;
...
//			}
	...
	protected static int translateKeyCode (int keyCode) {
		if (keyCode == com.jogamp.newt.event.KeyEvent.VK_ESCAPE)  return Keys.ESCAPE;
